    ...
    <div class="container-fluid p-5">
        ...
        <div class="col-12 px-0 table-box">
            <div class="table-top-panel d-flex align-items-center justify-content-between px-2 py-3">
                ...
                <a href="{{route('category.create')}}" class="btn adding-button">
                ...
                <table class="table table-hover" id="org_table">
                    ...
                    @foreach($categories as $key => $category)
                        ...
                        <td class="darkblue-color d-flex align-items-center justify-content-end">
                            <a href="{{route('category.edit', $category->id)}}" class="btn btn-outline-primary mr-3 text-nowrap">Update</a>
                        </td>
                        <td class="darkblue-color text-center text-nowrap align-middle"><a href="{{route('category.show',$category->id)}}">{{$category->title}}</a></td>
                        <td class="darkblue-color text-center text-nowrap align-middle">{{$category->min_description}}</td>
                        <td class="darkblue-color text-center text-nowrap align-middle">{{$category->description}}</td>
                        </td>
                        <td>
                            <form action="{{ route('category.destroy', $category->id)}}" method="post">
                                @method('DELETE')
                                @csrf
                                <input class="btn btn-outline-danger mr-3" type="submit" onclick="return confirm('{{$category->title}} Are you sure?')" value="Delete" />
                            </form>
                        </td>
                        </tr>
                    @endforeach
